/**
  ...
  Watches files and updates compilers, renderers and handlers
...
function addFile(router, file, options) {
  ...
  // !!! TODO pipeline from ext
  var pipeline;
  var source = { file: file };
  var renderer = Compiler.createRenderer(id, source, options.type, pipeline);
  var key = Utils.makeRouteKey(options.url, file, options);
  var route = Utils.makeRoute(options.url, file, options);
 
  var renderer = Compiler.createRenderer(key, source, options.type);
  var handler = Handler.createHandler(renderer);
