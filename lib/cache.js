...
var Cache = module.exports = {
  ...
  store: require("lru-cache")({
    ...
    max: _.get(global.shipp, "config.cache.sizeLimit", 32 * 1024 * 1024),
    ...
    length: function(cached, key) {
      ...
      // Object keys = 13 (string length of src, ttl, type, val), ttl = 4. Total = 17
      return size + 17 + Buffer.byteLength(key) + Buffer.byteLength(cached.src || "") + Buffer.byteLength(cached.type || "");
  ...
  **/
  ...
  middleware: function(req, res, next) {
    ...
    if (Cache.has(cacheKey)) {
      ...
      Utils.send(res, cached.type, cached.val, method, cached.ttl);
    } else {
      next();
    }
