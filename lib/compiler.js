/**
  ...
  - compiler caching
  - create pipeline
  - createRenderer uses source handler (getTemplate function)
  - createRenderer uses HTML
  - request caching
  - compiler.addPipeline(file.ext);
...
function createRenderer(id, source, type) {

  // Add pipeline if needed
  addPipeline(source.ext);
  ...
  // !!! TODO: CACHE
  renderFunctions[id] = {
    fn: createRenderFunction(id, type, metadata, compiler, query),
    metadata: metadata
  };

  return renderFunctions[id];
