...
function cache(method, id, contents, renderer) {
  ...
  if (!global.shipp.infograph.hasNode(`cache:${key}`)) {
    global.shipp.infograph.addNode(`cache:${key}`, "cache", () => Cache.cool(id, method))
    ...
    tables.forEach((table) => global.shipp.infograph.addEdge(`data:${table}`, `cache:${key}`))
  }
