...
Utils.makeRoute = function(baseRoute, file, options) {
  ...
  if (Utils.isTemplateFile(file, ext)) {
    route = route + "/:slug";
...
/**
  ...
  e.g. "/scripts/main.js" becomes "scripts:main-js"

  @param {String} route The route to key-ify

**/

Utils.makeKey = function(file) {
...
Utils.readDirectory = function(p, recursive, base) {
  ...
  fs.readdirSync(p).forEach(function(file) {
    ...
    if (/^\./.test(file) || (/^(includes?|imports?|helpers?|mixins?)/i.test(file))) return;
...
Utils.isTemplateFile = function(file, type) {
  return /^template$/i.test(file.name) && Utils.isHTML(type);
};
...
/**
  ...
  @param {Function} next Callback with results

**/

Utils.makeQueries = function(queries, context, next) {
