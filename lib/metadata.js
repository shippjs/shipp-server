...
Metadata.extract = function(str, directives) {
  ...
  Utils.getRegExpMatches(str, re, 1)

  .map(function(match) {
    return Metadata.parse(match, directives);
  })
...
Metadata.parse = function(str, directives) {

  var matches = str.trim().split(/\=/),
      name = matches.shift().trim(),
      type = directives[name],
      value;
  ...
  switch (type) {
    ...
    case "object":
      ...
      break;
    case "list":
      value = matches[0].trim().split(/\s*,\s*/);
      break;
    case "query":
    ...
    case "string":
      ...
      break;
    default:
      throw new Error("Unknown typeof directive " + type + " for " + name);
      break;
  }
